...
# 4. Thesaurus using Datamuse API (with antonym fallback)
def thesaurus_node(state: WordState) -> dict:
    ...
    # Fallback if no antonyms found
    if not antonyms:
        fallback = requests.get(f"https://api.datamuse.com/words?ml=opposite+of+{word}").json()
        antonyms = [a["word"] for a in fallback][:5]

    return {"synonyms": synonyms, "antonyms": antonyms}

# 5. Examples using dictionary scraping with inappropriate filter
from bs4 import BeautifulSoup
...
def generate_examples_node(state: WordState) -> dict:
    ...
    try:
        ...
        examples = []
        banned = ["kiss", "lover", "lips", "romantic", "sexual", "intercourse", "fuck", "seduce"]
        for item in soup.find_all("div", class_="sentence-item"):
            ...
            if sentence and word.lower() in sentence.text.lower():
                ...
                if not any(b in text.lower() for b in banned):
                    examples.append(text)

        if examples:
            return {"examples": examples[:3]}
        else:
            return {"examples": [f"No clean examples found for '{word}'."]}
